...
namespace FlyEngine
{
	namespace Importers
	{
		Entities::Model* ModelImporter::LoadModel(std::string const& path, bool gamma)
		{
		...
		Entities::Mesh ModelImporter::ProcessMesh(aiMesh* mesh, const aiScene* scene, Entities::Model* model)
		{
            ...
            // 1. diffuse maps
            std::vector<Entities::Texture> diffuseMaps = LoadMaterialTextures(material, aiTextureType_DIFFUSE, "texture_diffuse", model);
            textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());
            // 2. specular maps
            std::vector<Entities::Texture> specularMaps = LoadMaterialTextures(material, aiTextureType_SPECULAR, "texture_specular", model);
            textures.insert(textures.end(), specularMaps.begin(), specularMaps.end());
            // 3. normal maps
            std::vector<Entities::Texture> normalMaps = LoadMaterialTextures(material, aiTextureType_HEIGHT, "texture_normal", model);
            textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());
            // 4. height maps
            std::vector<Entities::Texture> heightMaps = LoadMaterialTextures(material, aiTextureType_AMBIENT, "texture_height", model);
            textures.insert(textures.end(), heightMaps.begin(), heightMaps.end());

            // return a mesh object created from the extracted mesh data
            return Entities::Mesh(vertices, indices, textures);
		}
		std::vector<Entities::Texture> ModelImporter::LoadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName, Entities::Model* model)
		{
            std::vector<Entities::Texture> textures;
